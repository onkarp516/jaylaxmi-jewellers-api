...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.service.EmployeeService;
...
public class EmployeeController {
    ...
    public Object addEmployeeDeviceId(@RequestBody Map<String, String> requestParam, HttpServletRequest request) {
        return employeeService.addEmployeeDeviceId(requestParam, request);
    }
    ...
    public ResponseEntity<?> createAuthenticateToken(@RequestBody Map<String, String> request, HttpServletRequest req) throws Exception {
        ...
        try {
            Employee userDetails = employeeRepository.findByMobileNumberAndDeviceIdAndTextPassword(username, deviceId, password);
            if(userDetails != null){
                ...
                Employee employee = employeeRepository.findByMobileNumberAndDeviceId(username, deviceId);
                if(employee != null){
                    responseMessage.setMessage("Please Enter Correct Password");
                    responseMessage.setResponseStatus(UNAUTHORIZED.value());
                } else {
                    Employee emp = employeeRepository.findByMobileNumberAndPassword(username, password);
                    if(emp != null){
                        if(emp.getDeviceId().equalsIgnoreCase("OS7JAY")){
                            emp.setDeviceId(deviceId);
                            emp.setStatus(true);
                            emp.setTextPassword(password);
                            String encPassword = passwordEncoder.encode(password);
                            emp.setPassword(encPassword);
                            employeeRepository.save(emp);
                            Object jwtToken = jwtTokenUtil.generateTokenForMobile(req, emp.getMobileNumber().toString());
                            responseMessage.setMessage("Login success");
                            responseMessage.setResponse(jwtToken);
                            responseMessage.setResponseStatus(HttpStatus.OK.value());
                        } else {
                            responseMessage.setMessage("Please Use Authorized Mobile Device");
                            responseMessage.setResponseStatus(UNAUTHORIZED.value());
                        }
                    } else {
                        Employee emp1 = employeeRepository.findByMobileNumber(Long.parseLong(username));
                        if(emp1 != null){
                            responseMessage.setMessage("Please Use Authorized Mobile Device");
                            responseMessage.setResponseStatus(UNAUTHORIZED.value());
                        } else {
