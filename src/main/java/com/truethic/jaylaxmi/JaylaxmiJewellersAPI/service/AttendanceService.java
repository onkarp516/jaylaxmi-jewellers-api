...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.fileConfig.FileStorageProperties;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.fileConfig.FileStorageService;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.viewRepository.AttendanceViewRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.AttendanceDTDTO;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.EmployeeDTDTO;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.GenericDTData;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.TaskDTO;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.views.AttendanceView;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.views.TaskView;
...
public class AttendanceService {
    ...
    static String[] empSalHEADERs = {"Employee Name", "Salary Per Month", "Present Days", "Leave Days", "Absent Days",
            "Days In Month", "Extra Days", "Extra Half Days", "Present Day Salary", "Absent Day Salary", "Extra Day Salary", "Worked Day Salary",
            "Overtime Count", "Overtime Amount", "Late Punch In Amount", "Advance Amount", "Salary Drawn"};
...
//                double wagesPoint = (wagesPerDay / 100.0);
                ...
                if(request.getParameterMap().containsKey("attendanceStatus")){
                    System.out.println("Request has attendance Status");
                } else {
                    System.out.println("Request does not contain attendance Status");
                }
...
//                            {
                                ...
                                if(workedHours > (employee.getShift().getWorkingHours().getHour()+employee.getShift().getWorkingHours().getMinute())) {
                                    LocalTime otDiff = utility.getTimeDiffFromTimes(employee.getShift().getToTime(), outTime.toLocalTime());
...
//                        }
                    ...
                    } else if (!attendance.getAttendanceDate().getDayOfWeek().toString().contains(employee.getWeeklyOffDay())){
                        absentDays++;
                    }
                }
            }
            ...
            String newUserDate = year + "-" + month + "-01";
...
//                            double basicAmt = (perDaySalary * totalDays) * 0.6;
                              basic = (employee.getExpectedSalary() * basicPer) / 100;
//                            basic = (basicAmt * (basicPer / 100.0));
                        } else if(payhead.getName().toLowerCase().contains("special")){
//                            double basicAmt = (perDaySalary * totalDays) * 0.4;
//                            specialAllowance = basicAmt * (payhead.getPercentage() / 100.0);
                            specialAllowance = (employee.getExpectedSalary() * payhead.getPercentage()) / 100;
                        }
                    }
                } else {
...
//            deductionAmount = deductionRepository.getSumOfDeduction();
            ...
            if(shift != null && lateCount > shift.getConsiderationCount()){
...
//                    daysToBeDeducted = lateCount / shift.getConsiderationCount();
            ...
            //temporarily commenting as per clients requirements
...
//                totalDeduction = (pf + esi + pfTax + latePunchDeductionAmt);
            ...
            if(deductionAmount > 0)
                totalDeduction = (pfTax + latePunchDeductionAmt + deductionAmount);
            else
                totalDeduction = (pfTax + latePunchDeductionAmt + advance);
...
//            res.addProperty("totalEmployees",totalEmployees);
    ...
    public JsonObject getSalaryReportMonthWise(Map<String, String> jsonRequest, HttpServletRequest request) {
        ...
        JsonArray jsonArray = new JsonArray();
        Users users = jwtTokenUtil.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        JsonArray salaryTotalArray = new JsonArray();
        JsonObject salaryTotal = new JsonObject();
        try {
            int daysInMonth = 0;
            System.out.println("jsonRequest " + jsonRequest.get("currentMonth"));
            String[] currentMonth = jsonRequest.get("currentMonth").split("-");
            ...
            JsonObject jsonObject = new JsonObject();
            double totalPresentAmt=0.0;
            double totalAbsentAmt=0.0;
            double totalExtraPayAmt=0.0;
            double totalLateAmt=0.0;
            double totalAdvanceAmt=0.0;
            double totalOTAmt=0.0;
            double totalAmt=0.0;
            double totalDrawnAmt=0.0;
...
//                                }
                    ...
                    Long otCount = attendanceRepository.getOvertimeCount(employee1.getId(), userYear, userMonth);
                    Double otAmount = 0.0;
                    double totalDaysOfEmployee = presentDays + extraDays + leaveDays + absentDays;
                    double presentDaysSalary = 0.0;
                    double absentDaysSalary = 0.0;
                    double extraDaysSalary = 0.0;
                    ...
                    extraDaysSalary = (extraDays + (extraHalfDays/2)) * wagePerDay;
                    if(otCount != null) {
                        otAmount = otCount * 1.0;
                        workedDaysSalary = presentDaysSalary + extraDaysSalary + otAmount;
//                        salaryDrawn = presentDaysSalary;
                    } else {
                        workedDaysSalary = presentDaysSalary + extraDaysSalary;
//                        salaryDrawn = presentDaysSalary;
                    }
                    ...
                    for(Payhead payhead : payheads){
                        if(payhead.getPayheadStatus()){                     // if this payhead is maraked as default
                            if(payhead.getPercentageOf() != null){
                                if(payhead.getName().toLowerCase().contains("basic")){
                                    basicPer = payhead.getPercentage();
                                    basic = (employee1.getExpectedSalary() * basicPer) / 100;
                                }
                            }
                        }
                    }
...
//                    for(Deduction deduction : deductions){
//                        if(deduction.getPercentageOf() != null){
//                            if(deduction.getName().toLowerCase().contains("pf") && employee1.getEmployeeHavePf() != null && employee1.getEmployeeHavePf()){
//                                pfPer = deduction.getPercentage();
//                                pf = (basic * (pfPer / 100.0));
//                            } else if(deduction.getName().toLowerCase().contains("esi") && employee1.getEmployeeHaveEsi() != null && employee1.getEmployeeHaveEsi()){
//                                esiPer = deduction.getPercentage();
//                                esi = (basic * (esiPer / 100.0));
//                            }
//                        }
//                    }
                    ...
                    if(pf != 0 && esi != 0){
                        salaryDrawn = workedDaysSalary - (pf+esi);
                    } else {
                        salaryDrawn = workedDaysSalary;
                    }

                    Long lateCount = attendanceRepository.getLateCount(employee1.getId(), userMonth);
                    double daysToBeDeducted = 0.0;
                    double hoursToBeDeducted = 0.0;
                    double latePunchDeductionAmt = 0.0;
                    Shift shift = shiftRepository.findByIdAndStatus(employee1.getShift().getId(), true);
                    if(shift != null && lateCount > shift.getConsiderationCount()){
                        if(shift.getIsDayDeduction()){
                            daysToBeDeducted = lateCount - 3;
                            if(shift.getDayValueOfDeduction().equalsIgnoreCase("quarter")) {
                                latePunchDeductionAmt = daysToBeDeducted * (wagePerDay/4);
                            } else if (shift.getDayValueOfDeduction().equalsIgnoreCase("half")) {
                                latePunchDeductionAmt = daysToBeDeducted * (wagePerDay/2);
                            } else {
                                latePunchDeductionAmt = daysToBeDeducted * wagePerDay;
                            }
                        } else {
                            hoursToBeDeducted = shift.getHourValueOfDeduction();
                            latePunchDeductionAmt = lateCount * (hoursToBeDeducted * wagesPerHour);
                        }
                    }
                    ...
                    try {
                        ledgerMaster = ledgerMasterRepository.findByEmployeeIdAndStatus(employee1.getId(), true);
                        if(ledgerMaster != null) {
                            Double openingBalance = ledgerMasterRepository.findOpeningBalance(ledgerMaster.getId());
                            sumCR = ledgerTransactionPostingsRepository.findsumCRForGivenMonth(ledgerMaster.getId(), String.valueOf(currentDate.getMonthValue()), String.valueOf(currentDate.getYear()));//-0.20
                            sumDR = ledgerTransactionPostingsRepository.findsumDRForGivenMonth(ledgerMaster.getId(), String.valueOf(currentDate.getMonthValue()), String.valueOf(currentDate.getYear()));//-0.40
                    ...
                    salaryDrawn = salaryDrawn - (latePunchDeductionAmt+Math.abs(closingBalance));
                    ...
                    empObj.addProperty("workedDaysSalary", workedDaysSalary);
                    empObj.addProperty("salaryDrawn", salaryDrawn);
                    empObj.addProperty("overtimeCount", otCount!=null?otCount:0);
                    empObj.addProperty("overtimeAmount", otAmount);
                    empObj.addProperty("latePunchDeductionAmt", latePunchDeductionAmt);
                    empObj.addProperty("advanceAmt", Math.abs(closingBalance));
//                    empObj.addProperty("pf", pf);
//                    empObj.addProperty("esi", esi);
                    empObj.addProperty("designation", employee1.getDesignation().getName());

                    totalPresentAmt += presentDaysSalary;
                    totalAbsentAmt += absentDaysSalary;
                    totalExtraPayAmt += extraDaysSalary;
                    totalLateAmt += latePunchDeductionAmt;
                    totalAdvanceAmt += Math.abs(closingBalance);
                    totalOTAmt += otAmount;
                    totalAmt += workedDaysSalary;
                    totalDrawnAmt += salaryDrawn;
                    salaryTotal.addProperty("totalPresentAmt",totalPresentAmt);
                    salaryTotal.addProperty("totalAbsentAmt",totalAbsentAmt);
                    salaryTotal.addProperty("totalExtraPayAmt",totalExtraPayAmt);
                    salaryTotal.addProperty("totalLateAmt",totalLateAmt);
                    salaryTotal.addProperty("totalAdvanceAmt",totalAdvanceAmt);
                    salaryTotal.addProperty("totalOTAmt",totalOTAmt);
                    salaryTotal.addProperty("totalAmt",totalAmt);
                    salaryTotal.addProperty("totalDrawnAmt",totalDrawnAmt);
                    ...
                    String query = "SELECT * from attendance_view WHERE status=1 AND institute_id="+users.getInstitute().getId()+
                            " AND attendance_date between '" + firstDateOfMonth + "' AND '" + lastDateOfMonth + "'"+
...
//                                }
                    ...
                    Long otCount = attendanceRepository.getOvertimeCount(employee.getId(), userYear, userMonth);
                    Double otAmount = 0.0;
                    double totalDaysOfEmployee = presentDays + extraDays + leaveDays + absentDays;
                    double presentDaysSalary = 0.0;
                    double absentDaysSalary = 0.0;
...
//                        salaryDrawn = presentDaysSalary;
                    ...
                    //this code is to calculate pf and esi
                    List<Deduction> deductions = deductionRepository.findAllByStatus();
//                    for(Deduction deduction : deductions){
//                        if(deduction.getPercentageOf() != null){
//                            if(deduction.getName().toLowerCase().contains("pf") && employee.getEmployeeHavePf() != null && employee.getEmployeeHavePf()){
...
//                    }
                    ...
                    Long lateCount = attendanceRepository.getLateCount(employee.getId(), userMonth);
                    double daysToBeDeducted = 0.0;
                    double hoursToBeDeducted = 0.0;
                    double latePunchDeductionAmt = 0.0;
                    Shift shift = shiftRepository.findByIdAndStatus(employee.getShift().getId(), true);
                    ...
                    LedgerMaster ledgerMaster = null;
                    try {
                        ledgerMaster = ledgerMasterRepository.findByEmployeeIdAndStatus(employee.getId(), true);
                        if(ledgerMaster != null) {
                            Double openingBalance = ledgerMasterRepository.findOpeningBalance(ledgerMaster.getId());
                            sumCR = ledgerTransactionPostingsRepository.findsumCRForGivenMonth(ledgerMaster.getId(), String.valueOf(currentDate.getMonthValue()), String.valueOf(currentDate.getYear()));//-0.20
                            sumDR = ledgerTransactionPostingsRepository.findsumDRForGivenMonth(ledgerMaster.getId(), String.valueOf(currentDate.getMonthValue()), String.valueOf(currentDate.getYear()));//-0.40
                    ...
                    jsonObject.addProperty("workedDaysSalary", workedDaysSalary);
                    jsonObject.addProperty("overtimeCount", otCount!=null?otCount:0);
                    jsonObject.addProperty("overtimeAmount", otAmount);
                    jsonObject.addProperty("latePunchDeductionAmt", latePunchDeductionAmt);
                    jsonObject.addProperty("advanceAmt", Math.abs(closingBalance));
//                    jsonObject.addProperty("pf", pf);
//                    jsonObject.addProperty("esi", esi);
                    jsonObject.addProperty("designation", employee.getDesignation().getName());
                    jsonObject.addProperty("totalDaysOfEmployee", totalDaysOfEmployee);
            ...
            salaryTotalArray.add(salaryTotal);
            res.add("list", jsonArray);
            res.add("salaryTotalArray", salaryTotalArray);
            response.add("response", res);
            response.addProperty("responseStatus", HttpStatus.OK.value());
        } catch (Exception e) {
            attendanceLogger.error("attendnaceList " + e);
...
//            jsonObj.addProperty("workingHours", attendanceView.getWorkingHours() != null ? Precision.round(attendanceView.getWorkingHours(), 2) : 0);
            ...
            if(attendanceView.getCheckOutTime() != null){
                ...
                double actualWorkingHoursInMinutes = totalMinutes - Precision.round(attendanceView.getLunchTime(), 2);
                jsonObj.addProperty("actualWorkingHoursInMinutes", String.valueOf(actualWorkingHoursInMinutes));
            } else {
...
//                    jsonObject.addProperty("totalTime", attendance.getTotalTime() != null ? String.valueOf(attendance.getTotalTime()) : "");
                    ...
                    if (!requestParam.get("checkOutTime").equalsIgnoreCase("")) {
                        ...
                        if (runningTask == null) {
                            if (attendance.getEmployee().getDesignation().getCode().equalsIgnoreCase("l3") ||
                                ...
                                firstTaskStartTime = checkInTime;
                                lastTaskEndTime = checkOutTime;
                                /* From Attendance Data */
                                totalTime = utility.getDateTimeDiffInTime(attendance.getCheckInTime(), lastTaskEndTime);
                                System.out.println("totalTime =>>>>>>>>>>>>>>>>>>>>>>" + totalTime);
                                attendance.setTotalTime(totalTime);
                                ...
                                if(workedHours > (attendance.getEmployee().getShift().getWorkingHours().getHour()+attendance.getEmployee().getShift().getWorkingHours().getMinute())) {
                                    LocalTime otDiff = utility.getTimeDiffFromTimes(attendance.getEmployee().getShift().getToTime(), checkOutTime.toLocalTime());
                                    int hour = otDiff.getHour();
                                    System.out.println("\nHour is = " + hour);
                                    int minute = otDiff.getMinute();
                                    System.out.println("Minute is = " + minute);
                                ...
                                WorkBreak workBreak = workBreakRepository.findByBreakName(attendance.getEmployee().getInstitute().getId());
                                TaskMaster taskMaster = null;
                                if (workBreak != null) {
                                    lunchTimeInMin = taskMasterRepository.getSumOfLunchTime(attendance.getId(), workBreak.getId(), true, false);
                                }
    ...
    public JsonObject getSalaryObjectInJsonObject(Employee employee1, List<LocalDate> localDates, LocalDate currentDate,String userYear, String userMonth, int daysInMonth){
...
//                    empObj.addProperty("esi", esi);
    ...
    public InputStream getEmployeeSalaryReportInExcel(String employeeId, String currentMonthByUser, HttpServletRequest req) {
        ...
        try {
            ...
            if(employeeId.equalsIgnoreCase("all")){
                List<Employee> employees = employeeRepository.findByInstituteIdAndStatusOrderByFirstNameAsc(users.getInstitute().getId(), true);
                for (Employee employee1 : employees) {
                    jsonArray.add(this.getSalaryObjectInJsonObject(employee1,localDates,currentDate,userYear,userMonth,daysInMonth));
                }
    ...
    private ByteArrayInputStream convertSalaryReportToExcel(JsonArray jsonArray) {
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet(empSalSHEET);
            ...
            for (JsonElement jsonElement : jsonArray) {
                ...
                try {
                    ...
                    row.createCell(1).setCellValue(obj.get("salaryPerMonth").getAsString());
                    row.createCell(2).setCellValue(obj.get("presentDays").getAsString());
                    row.createCell(3).setCellValue(obj.get("leaveDays").getAsDouble());
                    row.createCell(4).setCellValue(obj.get("absentDays").getAsString());
                    row.createCell(5).setCellValue(obj.get("daysInMonth").getAsString());
                    row.createCell(6).setCellValue(obj.get("extraDays").getAsString());
                    row.createCell(7).setCellValue(obj.get("extraHalfDays").getAsString());
                    row.createCell(8).setCellValue(obj.get("presentDaysSalary").getAsDouble());
                    row.createCell(9).setCellValue(obj.get("absentDaysSalary").getAsDouble());
                    row.createCell(10).setCellValue(obj.get("extraDaysSalary").getAsDouble());
                    row.createCell(11).setCellValue(obj.get("workedDaysSalary").getAsDouble());
                    row.createCell(12).setCellValue(obj.get("overtimeCount").getAsDouble());
                    row.createCell(13).setCellValue(obj.get("overtimeAmount").getAsDouble());
                    row.createCell(14).setCellValue(obj.get("latePunchDeductionAmt").getAsDouble());
                    row.createCell(15).setCellValue(obj.get("advanceAmt").getAsDouble());
                    row.createCell(16).setCellValue(obj.get("salaryDrawn").getAsDouble());
                } catch (Exception e) {
                    e.printStackTrace();
                    System.out.println("Exception e");
                }
...
//                        attendanceView.getAttendanceStatus() : "pending");
                ...
                if(attendanceView.getCheckOutTime() != null) {
                    ...
                    if (workedHours > employee.getShift().getWorkingHours().getHour()) {
                        LocalTime overtime = utility.getTimeDiffFromTimes(employee.getShift().getToTime(), LocalTime.parse(attendanceView.getCheckOutTime().format(DateTimeFormatter.ofPattern("HH:mm:ss"))));
            ...
            if (attendance == null) {
                ...
                if (wagesPerDay == null) {
                    ...
                    if (!requestParam.get("checkOutTime").equalsIgnoreCase("")) {
                        ...
                        if(workedHours > 11) {
                            LocalTime otDiff = utility.getTimeDiffFromTimes(employee.getShift().getToTime(), localDateTime.toLocalTime());
                            int hour = otDiff.getHour();
                            System.out.println("\nHour is = " + hour);
                            int minute = otDiff.getMinute();
                            System.out.println("Minute is = " + minute);
...
//                    jsonObj.addProperty("totalTime", attendance.getTotalTime().format(DateTimeFormatter.ofPattern("HH:mm:ss")));
                    ...
                    if(attendance.getCheckOutTime() != null)
                        totalTime = utility.getDateTimeDiffInTime(attendance.getCheckInTime(), attendance.getCheckOutTime());
                    jsonObj.addProperty("totalTime", totalTime != null ? totalTime.toString(): "");
                    array.add(jsonObj);
                }
            } else {
                Long employeeId = Long.parseLong(jsonRequest.get("employeeId"));
...
//        DateTimeFormatter myFormatObj = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        try{
            ...
            Double present = attendanceRepository.getPresentEmployeeCount(date, Long.parseLong(user.getInstitute().getId().toString()));
            Double empCount = employeeRepository.getEmployeeCount(true, user.getInstitute().getId());
...
//            JsonArray notLoggedArray = new JsonArray();
            ...
            if(jsonRequest.get("attendanceDate") != null)
                ...
                attendanceDate = LocalDate.now().toString();
            allEmployees = employeeRepository.findByStatus(true);
            ...
            for(Attendance attendance : presentEmployees){
                ...
                object.addProperty("designation",attendance.getEmployee().getDesignation().getName());
            ...
            for(Employee employee : employeesOnLeave){
                ...
                object.addProperty("designation",employee.getDesignation().getName());
