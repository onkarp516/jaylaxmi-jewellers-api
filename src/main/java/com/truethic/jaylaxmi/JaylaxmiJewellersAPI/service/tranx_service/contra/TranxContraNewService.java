package com.truethic.jaylaxmi.JaylaxmiJewellersAPI.service.tranx_service.contra;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.ContraMasterDTDTO;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.contra_repository.TranxContraMasterRepository;
...
//        Long count = tranxContaMasterRepository.findLastRecord(users.getOutlet().getId());
        ...
        if (users.getSite() != null) {
            count = tranxContaMasterRepository.findSiteLastRecord(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject createContra(HttpServletRequest request) {
        ...
        try {
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                tranxContraDetails.setSite(site);
...
//                tranxContraDetails.setLedgerName(contraRow.get("perticulars").getAsJsonObject().get("ledger_name").getAsString());
    ...
    private void insertIntoPostings(TranxContraDetails mContra, double total_amt, String crdrType,
        ...
        try {
            ...
            ledgerCommonPostings.callToPostings(total_amt, mContra.getLedgerMaster(), tranxType,
                    ...
                    mContra.getSite(), mContra.getCompany(), mContra.getTranxContraMaster().getTranscationDate(),
    ...
    private void saveIntoDayBook(TranxContraDetails mContra) {
        ...
        if (mContra.getSite() != null)
            dayBook.setSite(mContra.getSite());
    ...
    public JsonObject contraListbyCompany(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            contra = tranxContaMasterRepository.
                    findByCompanyIdAndSiteIdAndStatusOrderByIdDesc(users.getCompany().getId(), users.getSite().getId(), true);
        } else {
            contra = tranxContaMasterRepository.
                    findByCompanyIdAndStatusAndSiteIsNullOrderByIdDesc(users.getCompany().getId(), true);
        }
        ...
        try {
            ...
            if (mContra.getType().equalsIgnoreCase("dr")) {
                if (detailsId != 0) {
                    ...
                    ledgerCommonPostings.callToPostings(total_amt, mContra.getLedgerMaster(), tranxType,
                            mContra.getLedgerMaster().getAssociateGroups(), mContra.getTranxContraMaster().getFiscalYear(), mContra.getSite(),
...
//                        "AND tranx_receipt_master_tbl.status=1";
                query="SELECT * FROM `tranx_contra_master_tbl` WHERE company_id="+users.getCompany().getId()+" AND site_id="+users.getSite().getId()+" AND status=1 ORDER BY id DESC";
            } else {
