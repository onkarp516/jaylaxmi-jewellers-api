package com.truethic.jaylaxmi.JaylaxmiJewellersAPI.service.tranx_service.receipt;

import com.google.gson.Gson;
import com.google.gson.JsonArray;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.common.GenerateDates;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.common.GenerateFiscalYear;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.common.GenerateSlugs;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.ReciptMasterDTDTO;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.EmployeePayrollRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.EmployeeRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.ledgerdetails_repo.LedgerBalanceSummaryRepository;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.master_repo.TransactionStatusRepository;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.report_repository.DaybookRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.receipt_repository.TranxReceiptMasterRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.receipt_repository.TranxReceiptPerticularsDetailsRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.receipt_repository.TranxReceiptPerticularsRepository;
...
//        Long count = repository.findLastRecord(users.getCompany()().getId());
        ...
        if (users.getSite() != null) {
            count = repository.findSiteLastRecord(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject getSundryDebtorsAndIndirectIncomes(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            sundryCreditors = ledgerMasterRepository.findByCompanyIdAndSiteIdAndPrincipleGroupsIdAndStatus(
                    users.getCompany().getId(), users.getSite().getId(), 5L, true);
            sundryDebtors = ledgerMasterRepository.findByCompanyIdAndSiteIdAndPrincipleGroupsIdAndStatus(
                    users.getCompany().getId(), users.getSite().getId(), 1L, true);

        } else {
            sundryCreditors = ledgerMasterRepository.findByCompanyIdAndPrincipleGroupsIdAndStatusAndSiteIsNull(users.getCompany().getId(), 5L, true);
            sundryDebtors = ledgerMasterRepository.findByCompanyIdAndPrincipleGroupsIdAndStatusAndSiteIsNull(users.getCompany().getId(), 1L, true);
        }
                ...
                try {
                    ...
                    if(employee != null){
                        ...
                        if (employeePayroll != null) {
                            ...
                            for(LedgerMaster ledgerMaster: list) {
                                if (ledgerMaster.getLedgerName().equals("Basic Salary A/c") || ledgerMaster.getLedgerName().equals("Special Allowance A/c") || ledgerMaster.getLedgerName().equals("PF A/C") || ledgerMaster.getLedgerName().equals("ESIC A/C") || ledgerMaster.getLedgerName().equals("PT A/C") || ledgerMaster.getLedgerName().equals("Insentive") || ledgerMaster.getLedgerName().equals("Late Punch-In") ) {
                                    response.addProperty(utility.getKeyName(ledgerMaster.getLedgerName(), false), ledgerMaster.getLedgerName());
                                    response.addProperty(utility.getKeyName(ledgerMaster.getLedgerName(), true), ledgerMaster.getId());
                                }
                            }
                        }
                    }
...
//                    if (users.getSite() != null) {
//                        salesInvoice = tranxSalesInvoiceRepository.findPendingBillsBySiteId(users.getCompany().getId(), users.getSite().getId(), true, ledgerId);
//                    } else {
//                        salesInvoice = tranxSalesInvoiceRepository.findPendingBills(users.getCompany().getId(), true, ledgerId);
//                    }
...
//                    }
//                }
//                list = tranxCreditNoteNewReferenceRepository.findBySundryDebtorsIdAndStatusAndTransactionStatusIdAndAdjustmentStatusAndCompanyId(ledgerId, true, 1L, "credit", users.getCompany().getId());
...
//                listdebit = tranxDebitNoteNewReferenceRepository.findBySundryCreditorIdAndStatusAndTransactionStatusIdAndAdjustmentStatusAndCompanyId(ledgerId, true, 1L, "refund", users.getCompany().getId());
...
//    }
    ...
    public JsonObject createReceipt(HttpServletRequest request) {
        ...
        Site branch = null;
        if (users.getSite() != null) {
            branch = users.getSite();
            tranxReceipt.setSite(branch);
        }
        ...
        try {
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                tranxReceiptPerticulars.setSite(branch);
...
//                            TranxSalesInvoice mSaleInvoice = null;
                            tranxRptDetails.setSite(branch);
...
//                            }
    ...
    private void insertIntoPostings(TranxReceiptPerticulars mReceipt, double total_amt, String crdrType, String operation) {
        ...
        try {
            ...
            ledgerCommonPostings.callToPostings(total_amt, mReceipt.getLedgerMaster(), tranxType,
                    ...
                    mReceipt.getTranxReceiptMaster().getFiscalYear(), mReceipt.getSite(),
    ...
    private void saveIntoDayBook(TranxReceiptPerticulars mReceipt) {
        ...
        if (mReceipt.getSite() != null) dayBook.setSite(mReceipt.getSite());
    ...
    public Object DTReceiptList(Map<String, String> request, HttpServletRequest httpServletRequest) {
        ...
        try {
            if (users.getSite() != null) {
//                receipt = repository.findByCompanyIdAndSiteIdAndStatusOrderByIdDesc(users.getCompany().getId(), users.getSite().getId(), true);
//                query = "SELECT * FROM `tranx_receipt_master_tbl` WHERE company_id="+users.getCompany().getId()+" AND site_id="+users.getSite().getId()+" " +
//                        "AND tranx_receipt_master_tbl.status=1";
                query = "SELECT m.*, p.* FROM tranx_receipt_master_tbl m LEFT JOIN tranx_receipt_perticulars_tbl p " +
                        ...
                        "p.site_id="+users.getSite().getId()+" AND m.status=1 GROUP BY m.receipt_no";
            } else {
//                receipt = repository.findByCompanyIdAndStatusAndSiteIsNullOrderByIdDesc(users.getCompany().getId(), true);
