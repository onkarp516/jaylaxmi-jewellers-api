...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.ClientDetails;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.ClientsListDTO;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.UserRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.ledgerdetails_repo.*;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.master_repo.*;
...
//            principles = principleRepository.findByIdAndStatus(Long.parseLong(request.getParameter("principle_id")), true);
        ...
        ledgerMaster.setInstitute(users.getInstitute());
        ...
        if (users.getSite() != null) ledgerMaster.setSite(users.getSite());
...
//            ledgerMaster.setOpeningBal(0.0);
    ...
    public JsonObject getSundryCreditors(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            sundryCreditors = ledgerRepository.findSundryCreditorsByCompanyIdAndSiteId(users.getCompany().getId(), users.getSite().getId());
        } else {
...
//                users.getCompany().getId());
        ...
        if (users.getSite() != null) {
            sundryDebtors = ledgerRepository.findSundryDebtorsByCompanyIdAndSiteId(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject getPurchaseAccount(HttpServletRequest request) {
     ...
     /*   if (users.getBranch() != null) {
        ...
        result = getLedgers("Purchase Accounts", users.getCompany().getId(), users.getSite());
        JsonObject response = new JsonObject();
        if (result.size() > 0) {
            response.addProperty("message", "success");
            response.addProperty("responseStatus", HttpStatus.OK.value());
    ...
    private JsonArray getLedgers(String key, Long outletId, Site site) {
        ...
        if (site != null) {
            indirect_incomes = ledgerRepository.findByCompanyIdAndSiteIdAndPrinciplesIdAndStatus(outletId, site.getId(), principles.getId(), true);

        } else {
            indirect_incomes = ledgerRepository.findByCompanyIdAndPrinciplesIdAndStatusAndSiteIsNull(outletId, principles.getId(), true);
        }
        JsonArray result = new JsonArray();
...
//        result = getLedgers("Sales Accounts", users.getCompany().getId());
    /*    if (users.getBranch() != null) {
        ...
        result = getLedgers("Sales Accounts", users.getCompany().getId(), users.getSite());
        JsonObject response = new JsonObject();
        if (result.size() > 0) {
            response.addProperty("message", "success");
            response.addProperty("responseStatus", HttpStatus.OK.value());
...
//        result = getLedgers("Indirect Income", users.getCompany().getId());
     /*   if (users.getBranch() != null) {
        ...
        result = getLedgers("Indirect Income", users.getCompany().getId(), users.getSite());
        JsonObject response = new JsonObject();
        if (result.size() > 0) {
            response.addProperty("message", "success");
            response.addProperty("responseStatus", HttpStatus.OK.value());
...
//        result = getLedgers("Indirect Expenses", users.getCompany().getId());
      /*  if (users.getBranch() != null) {
        ...
        result = getLedgers("Indirect Expenses", users.getCompany().getId(), users.getSite());
        JsonObject response = new JsonObject();
        if (result.size() > 0) {
            response.addProperty("message", "success");
            response.addProperty("responseStatus", HttpStatus.OK.value());
    ...
    public JsonObject getAllLedgers(HttpServletRequest request) {
        ...
        try{
            if (users.getSite() != null) {
                ...
                balanceSummaries = ledgerRepository.findByCompanyIdAndSiteIdAndStatusOrderByIdDesc(users.getCompany().getId(), users.getSite().getId(), true);
            } else {
                balanceSummaries = ledgerRepository.findByCompanyIdAndStatusAndSiteIsNullOrderByIdDesc(users.getCompany().getId(), true);
            }
        }catch (Exception e){
...
/*    public String exportReport(HttpServletRequest request) throws IOException, JRException, URISyntaxException {
        ...
        if (users.getBranch() != null) {
            ledgerMasters = ledgerRepository.findTop3ByCompanyIdAndSiteIdOrderByIdDesc(users.getCompany().getId(), users.getBranch().getId());
        } else {
    ...
    public JsonObject getTotalAmountBillbyBill(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            list = transactionDetailsRepository.findByLedgerMasterIdAndCompanyIdAndSiteIdAndTransactionTypeId(Long.parseLong(request.getParameter("id")), users.getCompany().getId(), users.getSite().getId(), 1L);
        } else {
...
//            jsonObject.addProperty("drcr_type", mLedger.getOpeningBalType());4
    ...
    public Object getClientList(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            cashInHands = ledgerRepository.findCashInHandByCompanyIdAndSite(users.getCompany().getId(), users.getSite().getId());
        } else {
        ...
        if (users.getSite() != null) {
            bankAccounts = ledgerRepository.findBankAccountsByCompanyIdAndSite(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject getCashAcBankAccount(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            ...
            ledgerMaster = ledgerRepository.findLedgerIdAndSiteIdAndName(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject getCounterCustomer(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            sundryDebtors = ledgerRepository.findByLedgerNameIgnoreCaseAndCompanyIdAndSiteIdAndStatus("Counter Customer", users.getCompany().getId(), users.getSite().getId(), true);
        } else {
            sundryDebtors = ledgerRepository.findByLedgerNameIgnoreCaseAndCompanyIdAndStatusAndSiteIsNull("Counter Customer", users.getCompany().getId(), true);
        }
        JsonArray result = new JsonArray();
        JsonObject response = new JsonObject();
        if (sundryDebtors != null) {
    ...
    public Object validateLedgerMaster(HttpServletRequest request) {
        ...
        LedgerMaster lMaster1 = null;
        Map<String, String[]> paramMap = request.getParameterMap();
        if (users.getSite() != null) branchId = users.getSite().getId();
          /*  lMaster = ledgerRepository.findByCompanyIdAndSiteIdAndLedgerNameIgnoreCaseAndStatus(users.getCompany().getId(), users.getBranch().getId(), request.getParameter("ledger_name"), true);
        } else {
...
//            purInvoiceLogger.error("Error in purchaseDelete()->" + e.getMessage());
    ...
    public Object ledgerTransactionsList(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            query = query + " AND branch_id=" + users.getSite().getId();
        }
    ...
    public Object ledgerTransactionsDetails(HttpServletRequest request) {
        ...
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        if (users.getSite() != null) {
            ledgerMasters = ledgerRepository.findByCompanyIdAndSiteIdAndStatusAndId(users.getCompany().getId(), users.getSite().getId(), true, ledgerId);
        } else {
            ledgerMasters = ledgerRepository.findByCompanyIdAndStatusAndIdAndSiteIsNull(users.getCompany().getId(), true, ledgerId);
        }
    ...
    public JsonObject getPayrollLedgers(HttpServletRequest request) {
        ...
        JsonArray result = new JsonArray();
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        List<LedgerMaster> list = ledgerRepository.findByCompanyIdAndSiteIdAndInstituteIdAndStatus(users.getCompany().getId(),users.getSite().getId(),users.getInstitute().getId(), true);
