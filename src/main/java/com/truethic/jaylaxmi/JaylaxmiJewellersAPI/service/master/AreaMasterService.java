...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.master_repo.AreaMasterRepository;
...
public class AreaMasterService {
    ...
    public JsonObject getAllAreaMaster(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            list = areaMasterRepository.findByCompanyIdAndStatusAndSiteId(outletId, true, users.getSite().getId());
        } else {
            list = areaMasterRepository.findByCompanyIdAndStatusAndSiteIsNull(outletId, true);
        }
        if (list.size() > 0) {
