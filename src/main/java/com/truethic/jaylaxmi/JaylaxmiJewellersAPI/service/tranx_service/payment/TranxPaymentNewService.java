package com.truethic.jaylaxmi.JaylaxmiJewellersAPI.service.tranx_service.payment;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.dto.PaymentMasterDTDTO;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.ledgerdetails_repo.LedgerTransactionDetailsRepository;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.payment_repository.TranxPaymentMasterRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.payment_repository.TranxPaymentPerticularsDetailsRepository;
...
//    private TranxCreditNoteNewReferenceRepository tranxCreditNoteNewReferenceRepository;
    ...
    public JsonObject paymentLastRecord(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            count = tranxPaymentMasterRepository.findSiteLastRecord(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject getCashAcBankAccountDetails(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            ledgerMaster = ledgerMasterRepository.findSiteBankAccountCashAccount(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject createPayments(HttpServletRequest request) {
        ...
        try {
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                tranxPaymentPerticulars.setSite(branch);
                ...
                crdrType = paymentRow.get("type").getAsJsonObject().get("type").getAsString();
...
//                            TranxPurInvoice mPurInvoice = null;
                            tranxPymtDetails.setSite(branch);
...
//                                jsonObject.addProperty("dr", df.format(Math.abs(closingBalance)));
                                if (employeePayroll != null) {
                                    ...
                                    employeePayroll.setInstitute(ledgerMaster.getEmployee().getInstitute());
                                    employeePayroll.setYearMonth(yearMonth);
                                    employeePayroll.setAdvance(Math.abs(closingBalance));
                                }
    ...
    private void insertIntoPostings(TranxPaymentPerticulars paymentRows, double total_amt, String crdrType, String operation) {
        ...
        try {
            /**** New Postings Logic *****/
            ledgerCommonPostings.callToPostings(total_amt, paymentRows.getLedgerMaster(), tranxType, paymentRows.getLedgerMaster().getAssociateGroups(), paymentRows.getTranxPaymentMaster().getFiscalYear(), paymentRows.getSite(), paymentRows.getCompany(), paymentRows.getTranxPaymentMaster().getTranscationDate(), paymentRows.getTranxPaymentMaster().getId(), paymentRows.getTranxPaymentMaster().getPaymentNo(), crdrType, true, "Payment", operation);
            /**** Save into Day Book ****/
    ...
    private void saveIntoDayBook(TranxPaymentPerticulars paymentRows) {
        ...
        if (paymentRows.getSite() != null) dayBook.setSite(paymentRows.getSite());
...
////                    if (users.getSite() != null) {
////                        purInvoice = tranxPurInvoiceRepository.findPendingBillsBySite(users.getCompany().getId(), users.getSite().getId(), true, ledgerId);
////                    } else {
...
//    }
    ...
    public JsonObject paymentListbyCompany(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            payment = tranxPaymentMasterRepository.findByCompanyIdAndSiteIdAndStatusOrderByIdDesc(users.getCompany().getId(), users.getSite().getId(), true);
        } else {
            payment = tranxPaymentMasterRepository.findByCompanyIdAndStatusAndSiteIsNullOrderByIdDesc(users.getCompany().getId(), true);
        }
            ...
            if (paymentRows.getType().equalsIgnoreCase("dr")) {
                if (detailsId != 0) {
                    ...
                    ledgerCommonPostings.callToPostings(total_amt, paymentRows.getLedgerMaster(), tranxType,
                            ...
                            paymentRows.getTranxPaymentMaster().getFiscalYear(), paymentRows.getSite(),
...
//                        "AND tranx_receipt_master_tbl.status=1";
                query="SELECT m.*, p.* FROM tranx_payment_master_tbl m LEFT JOIN tranx_payment_perticulars_tbl p "+
                        ...
                        "p.site_id="+users.getSite().getId()+" AND m.status=1 GROUP BY m.payment_no";
            } else {
