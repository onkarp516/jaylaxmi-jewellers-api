package com.truethic.jaylaxmi.JaylaxmiJewellersAPI.service.tranx_service.gstinput;
...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.gstinput_repository.GstInputDetailsRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.gstinput_repository.GstInputDutiesTaxesRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.tranx_repository.gstinput_repository.GstInputMasterRepository;
...
//    private ProductHsnRepository productHsnRepository;
    ...
    public JsonObject gstInputLastRecord(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            count = gstInputMasterRepository.findSiteLastRecord(users.getCompany().getId(), users.getSite().getId());
        } else {
    ...
    public JsonObject createGstInput(HttpServletRequest request) {
        ...
        try {
            ...
            if (users.getSite() != null)
                roundoff = ledgerMasterRepository.findByCompanyIdAndSiteIdAndLedgerNameIgnoreCase(
                        users.getCompany().getId(), users.getSite().getId(), "Round off");
            else
                roundoff = ledgerMasterRepository.findByCompanyIdAndLedgerNameIgnoreCaseAndSiteIsNull(
...
//                gstInputDetails.setProductHsn(hsn);
            ...
            if (cgstList != null && cgstList.size() > 0) {
                for (JsonElement taxElement : cgstList) {
                    ...
                    if (tranxGstInput.getSite() != null)
                        dutiesTaxes = ledgerMasterRepository.findByCompanyIdAndSiteIdAndLedgerNameIgnoreCase(
                                tranxGstInput.getCompany().getId(), tranxGstInput.getSite().getId(), ledgerName);
                    else
                        dutiesTaxes = ledgerMasterRepository.findByCompanyIdAndLedgerNameIgnoreCaseAndSiteIsNull(
