...
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.ledgerdetails_repo.LedgerFormRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.ledgerdetails_repo.LedgerTransactionPostingsRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.master_repo.AssociateGroupsRepository;
import com.truethic.jaylaxmi.JaylaxmiJewellersAPI.repository.master_repo.FoundationRepository;
...
*/
    public Object createAssocitesgroup(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            siteId = users.getSite().getId();
            site = users.getSite();

        }
        if (validateAG(request.getParameter("associates_group_name"), users.getCompany().getId(), siteId, principles.getId(), pgroupId)) {
...
/*    public JsonObject getAssocitesgroup(HttpServletRequest request) {
  ...
  /*  public Object DTAssociateGroups(Map<String, String> request, HttpServletRequest req) {
    ...
    public JsonObject getAllAccountGroups(HttpServletRequest request) {
        ...
        // List<Principles> p_list = principleRepository.findAll();
        // List<PrincipleGroups> pg_list = repository.findAll();
        List<AssociateGroups> ag_list = associateGroupsRepository.findByCompanyIdAndStatus(users.getCompany().getId(),true);
        for (Principles mp : p_list) {
            JsonObject response = new JsonObject();
            response.addProperty("principle_id", mp.getId());
            response.addProperty("principle_name", mp.getPrincipleName());
    ...
    public JsonObject getAssocitesgroup(HttpServletRequest request) {
        ...
        if (users.getSite() != null) {
            ag_list = associateGroupsRepository.findByCompanyIdAndStatusAndSiteIdOrderByIdDesc(users.getCompany().getId(),
                   true, users.getSite().getId());
        } else {
            ag_list = associateGroupsRepository.findByCompanyIdAndStatusAndSiteIsNullOrderByIdDesc(users.getCompany().getId(), true);
        }
        JsonArray result = new JsonArray();
        for (AssociateGroups associates : ag_list) {
            JsonObject response = new JsonObject();
