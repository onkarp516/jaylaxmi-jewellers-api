...
//    Page<Employee> findByOrderByIdDesc(Pageable pageable);
    ...
    @Query(value = " SELECT IFNULL(COUNT(id),0.0) FROM employee_tbl as a WHERE a.status=?1 AND institute_id=?2", nativeQuery = true)
    Double getEmployeeCount(boolean b, Long instituteId);
