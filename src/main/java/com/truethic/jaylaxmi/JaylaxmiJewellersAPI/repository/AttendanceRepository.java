...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    @Query(value = "SELECT IFNULL(COUNT(att.id),0.0) FROM attendance_tbl att LEFT JOIN employee_tbl emp ON att.employee_id=emp.id " +
            ...
            nativeQuery = true)
    Double getPresentEmployeeCount(LocalDate b, Long instituteId);
    ...
    @Query(value = "SELECT SUM(overtime) AS overtime FROM `attendance_tbl` WHERE employee_id=?1 AND MONTH(attendance_date)=?3 AND YEAR(attendance_date)=?2", nativeQuery = true)
    Long getOvertimeCount(Long id, String userYear, String userMonth);

    @Query(value = "SELECT SUM(overtime_amount) AS amount FROM `attendance_tbl` WHERE employee_id=?1 AND MONTH(attendance_date)=?3 AND YEAR(attendance_date)=?2", nativeQuery = true)
