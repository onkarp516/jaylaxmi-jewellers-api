...
/*    @Procedure("LEDGER_TRANSACTION_DETAILS_POSTINGS_INSERT")
    void insertIntoLegerTranxDetailsPosting(Long foundation_id, Long principle_id, Long principle_group_id, Long associates_group_id, Long tranx_type_master_id, Long balancing_method_id, Long institute_id, Long site_id, Long company_id, String payment_status, double debit, double credit, LocalDate transactionDate, LocalDate payment_date, Long tranx_id, String transaction_name, String under_prefix, String financial_year, Long created_by, Long ledger_master_id);*/

    @Procedure("LEDGER_TRANSACTION_DETAILS_POSTINGS_INSERT")
    void insertIntoLegerTranxDetailsPosting(Long foundation_id, Long principle_id, Long principle_group_id, Long associates_group_id, Long tranx_type_master_id, Long balancing_method_id, Long site_id, Long company_id, String payment_status, Double debit, Double credit, LocalDate transactionDate, LocalDate payment_date, Long tranx_id, String transaction_name, String under_prefix, String financial_year, Long created_by, Long ledger_master_id, String voucher_no);
    ...
    @Query(
            value = "SELECT * FROM `ledger_transaction_details_tbl` where principle_groups_id IN (1,2,4,5) Or (principle_id IN(9,12) " +
                    "AND (transaction_type_id!=1 AND transaction_type_id!=3 AND transaction_type_id!=2 " +
                    "AND transaction_type_id!=4)) OR (principle_groups_id=6 AND principle_id=6 AND transaction_type_id=10)" + " AND company_id=?1 AND site_id=?2",
            nativeQuery = true
    )
    List<LedgerTransactionDetails> findbygroupIdAndSite(Long id, Long id1);

    List<LedgerTransactionDetails> findByLedgerMasterIdAndCompanyIdAndSiteIdAndTransactionTypeId(long id, Long id1, Long id2, long l);
    ...
    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0.0),IFNULL(ledger_transaction_details_tbl.principle_id,0) FROM `ledger_transaction_details_tbl` LEFT JOIN ledger_master_tbl ON " +
                    ...
                    "AND ledger_transaction_details_tbl.site_id=?2 AND ledger_transaction_details_tbl.status=?3 AND " +
    ...
    List<Object[]> findByDateWiseTotalAmountOuletAndSiteStatus(Long id, Long id1, boolean b, String diex, LocalDate startDate, LocalDate endDate);
    ...
    @Query(
            value = "SELECT SUM(closing_bal),principle_id  FROM `ledger_transaction_details_tbl` WHERE ledger_transaction_details_tbl.company_id=?1 AND ledger_transaction_details_tbl.site_id=?2 " +
    ...
    List<Object[]> findByDateWiseTotalAmountOuletAndSiteStatusStep1(Long id, Long id1, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);
    ...
    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0),  IFNULL(opening_bal,0) FROM `ledger_transaction_details_tbl` WHERE company_id=?1 AND site_id=?2 status=?3 AND" +
                    "ledger_master_id=?4 AND DATE(transaction_date) BETWEEN ?5 ANd ?6 ;", nativeQuery = true
    )
   Double findByTotalAmountByMonthStartDateAndEndDateAndSiteAndCompanyAndStatus(Long id, Long id1, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);
   ...
   Double findByTotalAmountByMonthStartDateAndEndDateAndCompanyAndStatus(Long id, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);
    ...
    @Query(
            value = "SELECT * FROM `ledger_transaction_details_tbl` WHERE company_id=?1 AND (site_id=?2 OR site_id IS NULL) AND  status=?3 AND " +
                    "ledger_master_id=?4 AND DATE(transaction_date) BETWEEN ?5 ANd ?6 ;", nativeQuery = true
    )
    List<LedgerTransactionDetails> findByIdAndCompanyIdAndSiteAndStatusStep3(Long id, Long id1, boolean b, Long ledger_master_id, LocalDate startDate, LocalDate endDate);
    ...
    @Query(
            value = "SELECT ledger_transaction_details_tbl.opening_bal FROM ledger_transaction_details_tbl WHERE company_id=?1 AND site_id=?2 AND status=?3 AND" +
                    " id=?4 AND transaction_date=?5", nativeQuery = true
    )
    Double findByIdAndCompanyIdAndSiteIdAndStatuslt(Long id, Long id1, boolean b, Long principle_id, LocalDate startDatep);
    @Query(
